...
from bin.lib.Astro_Libraries.Nebular_Continuum import NebularContinuumCalculator
...
Te          = 10000.0
wave        = np.arange(1200, 20000)
HeI,HeII    = 0.01, 0.001
Flux_recomb = 1.5e-13
  
neb.PropertiesfromUser(Te, Flux_recomb, HeI, HeII, wave)
 
neb.load_neb_constants(wave)

Gamma_FF_HI_fast    = neb.free_free_gCont(wave, Te)
Gamma_2q_HI_fast    = neb.two_photon_gCont(wave, Te)
Gamma_FB_HI_fast    = neb.free_bound_gCont(wave, Te, neb.HI_fb_dict)
Gamma_FB_HeI_fast   = neb.free_bound_gCont(wave, Te, neb.HeI_fb_dict)
Gamma_FB_HeII_fast  = neb.free_bound_gCont(wave, Te, neb.HeII_fb_dict)
