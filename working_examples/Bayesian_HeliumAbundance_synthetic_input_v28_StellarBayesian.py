...
from lib.Astro_Libraries.Abundances_InferenceModel_Helium_v40 import Run_MCMC
   
iterat, burn, thin  = 10000, 0, 1
sim_model           = 'Bayes_Stellar2'
sim_components      = '_stellar_tests'
obs_metals          = ['H1', 'He1', 'S2', 'S3', 'O2', 'O3', 'N2', 'Ar3', 'Ar4']
sim_name            = sim_model + sim_components
params_list         = ['sigma_star', 'Av_star'] 
burning             = 5000
...
# mean_trace = np.mean(stat_db_dict['ssp_spectrum']['trace'], axis=0)
# std_trace = np.std(stat_db_dict['ssp_spectrum']['trace'], axis=0)
# 
# for i in range(len(mean_trace)):
#     print i, mean_trace[i], std_trace[i]
# 
# dz.FigConf()
# dz.data_plot(bm.obj_data['obs_wave_resam'], bm.obj_data['obs_flux_norm'], label = 'obs_flux_norm')
# dz.data_plot(bm.obj_data['obs_wave_resam'], np.mean(stat_db_dict['ssp_spectrum']['trace'], axis=0),  label = 'ssp_spectrum')
# dz.FigWording(xlabel = 'Wavelength', ylabel = 'Flux', title = '')
# dz.display_fig()
