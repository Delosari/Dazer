...
class Inference_AbundanceModel(Import_model_data, Collisional_FluxCalibration, Recombination_FluxCalibration, Continua_FluxCalculation, Nebular_FluxCalculation):
    ...
    def calculate_simObservation(self, model, obs_lines, verbose = True):
        ...
        self.obs_recomb_fluxes      = self.synth_recomb_emission(obs_lines)
        self.obs_recomb_err         = self.obs_recomb_fluxes * 0.02
        ...
        self.Halpha_norm            = self.obs_recomb_fluxes[self.idx_Halpha][0] * self.obj_data['Hbeta_Flux'] / self.stellar_SED['normFlux_stellar']
    ...
    def convert_np_array(self, ssp_coefs):
        return array(ssp_coefs)
    
    def array_coeffs_calc(self):
        
        array_coeffs = empty(self.nBases)
        
        for i in self.range_bases:
            array_coeffs[i] = pymc2.Uniform('ssp_coefs_%i' % i,   0.0,  3.00)
        
        return array_coeffs
    
    def ssp_synthesis(self, ssp_coeffs, ssp_grid_i_masked):
        
        fitted_spectrum = np_sum(ssp_coeffs.T * ssp_grid_i_masked, axis=1)
