...
class Import_model_data(ReddeningLaws):
    ...
    def import_optical_depth_coeff_table(self, HeliumLines):
        ...
        for i in range(len(HeliumLines)):
            ...
            if Line != self.H13889A_label:
                ...
                Data_dict[self.He3889_label] = loadtxt(self.paths_dict['Helium_OpticalDepth'], dtype=float, skiprows=2,
                                                       usecols=(i,))
    ...
    def load_obs_data(self, lines_df, obj_series, extension_treat='', Deblend_Check=True):
        ...
        self.Hbeta_Eqw = nominal_values(
            lines_df.loc[idx_Hbeta, 'line_Eqw'].values)  # WARNING: in cases with removed stellar this eqw gives trouble
        ...
        self.obj_data['H_Eqw'] = nominal_values(
            lines_df.loc[idcs_H, 'line_Eqw'].values)  # WARNING: in cases with removed stellar this eqw gives trouble
        ...
        self.obj_data['He_Eqw'] = nominal_values(
            lines_df.loc[idcs_He, 'line_Eqw'].values)  # WARNING: in cases with removed stellar this eqw gives trouble
        ...
        self.obj_data['He_pyneb_code'] = lines_df.loc[idcs_He].index.str[
                                         lines_df.loc[idcs_He].index.str.find('_') + 1:-1]
        ...
        if 'H1_3889A' in lines_df.index:
            ...
            He3889A_debFlux = self.deblend_He3889A(Te=self.obj_data['TOIII'], ne=self.obj_data['nSII'],
                                                   cHbeta=self.obj_data['cHbeta_obs'])
            ...
            insert(self.obj_data['He_Eqw'], 0, nominal_values(lines_df.loc[
                                                                  idx_He3889A, 'line_Eqw'].values))  # WARNING: in cases with removed stellar this eqw gives trouble
            ...
            insert(self.obj_data['He_pyneb_code'], 0,
                   lines_df.loc[idx_He3889A].index.str[lines_df.loc[idx_He3889A].index.str.find('_') + 1:-1])
...
class Recombination_FluxCalibration(LineMesurer_v2):
    def __init__(self):
        ...
        posHelium_Lines = ['He1_3889A', 'He1_4026A', 'He1_4387A', 'He1_4471A', 'He1_4686A', 'He1_4714A', 'He1_4922A',
                           'He1_5876A', 'He1_6678A', 'He1_7065A', 'He1_7281A', 'He1_10830A']
    ...
    def calculate_H_params(self, Te, ne):
        ...
        for i in self.nHydrogen_range:
            self.data_dic['Emissivity_H_vector'][i] = self.H1.getEmissivity(Te, ne,
                                                                            label=self.obj_data['H_pyneb_code'][i])
    ...
    def calculate_He_params(self, Te, ne, tau):
        ...
        for i in self.nHelium_range:
            self.data_dic['Emissivity_He_vector'][i] = self.He1.getEmissivity(Te, ne,
                                                                              label=self.obj_data['He1_pyneb_code'][i])
            self.data_dic['ftau_He_vector'][i] = self.OpticalDepth_He(tau=tau, T_4=T_4, ne=ne,
                                                                      He_label=self.obj_data['He1_labels'][i])
    ...
    def Kalpha_Ratio_H(self, T_4, H_label):
        ...
        K_alpha_Ratio = sum(
            self.Coef_Kalpha_dict[H_label][0] * exp(self.Coef_Kalpha_dict[H_label][1] / T_4) * power(T_4,
                                                                                                     self.Coef_Kalpha_dict[
                                                                                                         H_label][2]))
    ...
    def OpticalDepth_He(self, tau, T_4, ne, He_label):
        ...
        f_tau = 1 + (tau / 2) * (self.Coef_ftau_dict[He_label][0] + (
        self.Coef_ftau_dict[He_label][1] + self.Coef_ftau_dict[He_label][2] * ne + self.Coef_ftau_dict[He_label][
            3] * ne * ne) * T_4)

        return f_tau

    def calculate_recomb_fluxes(self, Thigh, ne, cHbeta, xi, tau, lines_abund_dict, lines_waves, lines_ions,
                                lines_flambda):
...
class Collisional_FluxCalibration(Import_model_data):
    ...
    def calculate_colExcit_flux(self, Tlow, Thigh, ne, cHbeta, lines_abund_dict, lines_waves, lines_ions,
                                lines_flambda):
...
class Inference_AbundanceModel(Import_model_data, Collisional_FluxCalibration, Recombination_FluxCalibration,
                               Continua_FluxCalculation, Nebular_FluxCalculation):
    def __init__(self):
    ...
    def calculate_simObservation(self, model, obs_lines, verbose=True):
        ...
        self.Halpha_norm = self.recomb_fluxes[self.idx_Halpha][0] * self.obj_data['Hbeta_Flux'] / self.stellar_SED[
            'normFlux_stellar']

        self.nebular_SED = self.calculate_nebular_SED(self.stellar_SED['stellar_wave_resam'], self.obj_data['z_star'],
                                                      self.obj_data['cHbeta'], \
                                                      self.obj_data['T_low'], self.obj_data['He1_abund'],
                                                      self.obj_data['He2_abund'], self.Halpha_norm)
        ...
        self.obj_data['n_pixels'] = len( self.stellar_SED['stellar_wave_resam'])
    ...
    def synth_collisional_emission(self, obs_lines):
        ...
        colExcit_fluxes = self.calculate_colExcit_flux(self.obj_data['T_low'], self.obj_data['T_high'],
                                                       self.obj_data['n_e'], self.obj_data['cHbeta'], abund_dict,
                                                       lines_waves, lines_ions, lines_flambda)
    ...
    #
    #     @pymc2.deterministic()
    #     def calc_abund_dict(He1_abund=He1_abund, S2_abund=S2_abund, S3_abund=S3_abund, O2_abund=O2_abund,
    #                         O3_abund=O3_abund, N2_abund=N2_abund, Ar3_abund=Ar3_abund, Ar4_abund=Ar4_abund):
    ...
    #
    #     @pymc2.deterministic
    #     def calc_nebular_cont(z_star=self.obj_data['z_star'], cHbeta=self.obj_data['cHbeta'], Te=self.obj_data['T_low'],
    #                           He1_abund=He1_abund, He2_abund=0.0, Halpha_Flux=self.Halpha_norm):
    #         neb_flux_norm = self.nebular_Cont(self.obj_data['obs_wave_resam'], z_star, cHbeta, Te, He1_abund, He2_abund,
    #                                           Halpha_Flux)
    #
    #         return neb_flux_norm
    #
    #     @pymc2.deterministic
    #     def calc_continuum(z_star=self.obj_data['z_star'], Av_star=Av_star, sigma_star=sigma_star, ssp_coefs=ssp_coefs,
    ...
    #     def likelihood_ssp(value=self.obj_data['obs_flux_norm_masked'], fit_continuum=calc_continuum,
    #                        sigmaContinuum=self.obj_data['obs_fluxEr_norm']):
    ...
    #
    #     @pymc2.stochastic(observed=True)  # Likelihood
    #     def likelihood_recomb(value=self.recomb_fluxes, H_He_TheoFlux=calc_recomb_fluxes,
    #                           sigmaLines=self.obs_recomb_err):
    #         chi_F = sum(square(H_He_TheoFlux - value) / square(sigmaLines))
    #         return - chi_F / 2
    #
    #     @pymc2.stochastic(observed=True)  # Likelihood
    #     def likelihood_colExcited(value=self.obs_metal_fluxes, theo_metal_fluzes=calc_colExcit_fluxes,
    #                               sigmaLines=self.obs_metal_Error):
    ...
    def He_O_S_nebStellar_model(self, metals_features = True, recomb_features = True, stellar_continua = True, nebular_continua = True, ions_array = None):
        ...
        if nebular_continua or recomb_features or metals_features:
            ne      = pymc2.TruncatedNormal('ne', self.obj_data['nSII'], self.obj_data['nSII_error'] ** -2, a=50.0, b=1000.0)
            cHbeta  = pymc2.TruncatedNormal('cHbeta', 0.15, 0.05 ** -2, a=0.0, b=3.0)
        ...
        if ions_array is not None:

            abund_vector = []
            for i in range(len(ions_array)):
                abund_vector.append(pymc2.Uniform(ions_array[i], 0.000001, 0.001))
        ...
        @pymc2.deterministic()
        def calc_abund_dict(He1_abund=He1_abund, abund_array = abund_vector):

            self.abund_iter_dict['He1'] = He1_abund
            self.abund_iter_dict['S2'] = abund_array[0]
            self.abund_iter_dict['S3'] = abund_array[1]
            self.abund_iter_dict['O2'] = abund_array[2]
            self.abund_iter_dict['O3'] = abund_array[3]
            self.abund_iter_dict['N2'] = abund_array[4]
            self.abund_iter_dict['Ar3'] = abund_array[5]
            self.abund_iter_dict['Ar4'] = abund_array[6]
        ...
        def calc_nebular_cont(z_star=self.obj_data['z_star'], cHbeta=self.obj_data['cHbeta'], Te=self.obj_data['T_low'], He1_abund=He1_abund, He2_abund=0.0, Halpha_Flux=self.Halpha_norm):
            ...
            if nebular_continua:
                ...
                neb_flux_norm = zeros(self.obj_data['n_pixels'])
    ...
    def stellar_bayes(self):
        ...
        @pymc2.deterministic
        def ssp_spectrum(z_star=self.obj_data['z_star'], Av_star=Av_star, sigma_star=sigma_star,
                         ssp_coefs=ssp_coefs):  # , nebular_flux=nebular_continua):
        ...
        @pymc2.stochastic(observed=True)  # Likelihood
        def likelihood_ssp(value=self.obj_data['obs_flux_norm_masked'], StellarCont_TheoFlux=ssp_spectrum,
                           sigmaContinuum=self.obj_data['obs_fluxEr_norm']):
...
class Run_MCMC(Inference_AbundanceModel, ssp_fitter):
    ...
    def select_inference_model(self, model):
        ...
        if model == '_stellar_tests':
            ...
            metals_array = ['S2_abund', 'S3_abund', 'O2_abund', 'O3_abund', 'N2_abund', 'Ar3_abund', 'Ar4_abund']

            self.inf_dict = self.He_O_S_nebStellar_model(metals_features = True, recomb_features = True, stellar_continua = True, nebular_continua = True, ions_array = metals_array)
