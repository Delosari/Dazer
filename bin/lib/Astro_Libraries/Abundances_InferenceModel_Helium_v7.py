...
class HeAbundance_InferenceMethods(Txt_Files_Manager):
    ...
    def Import_OpticalDepth_Coeff_Table(self, HeliumLines):
        ...
        #Adding the blended HeI 3889 A line
        data_dict[self.He3889_label] = data_dict.pop(self.H13889A_label)     
...
#             print self.Cand_Helium_Lines[i], self.Emissivity_He_vector[i]/Helium_Dict['Hbeta_emis'] 
    ...
    def Import_Object_Data(self, Filefolder, CodeName, Lineslog_Extension, Deblend_Check = True):
        ...
        if TOIII_from_TSIII != None:
            ...
            self.ObjectData_dict['TOIII']       = 14000.0
    ...
    def Check_EmissionLinesObserved(self, Data_Dict):
        ...
        if self.H13889A_label in self.Cand_Helium_Lines:
            ...
            He_3889A_DeblendFlux = self.Deblend_He3889A_line(a_H = 1.0, T_4 = self.ObjectData_dict['TOIII'] / 10000, cHbeta = self.ObjectData_dict['cHbeta_obs'])
    ...
    def He_Flux_theo_nof(self, xi, cHbeta, y_plus, data_dict):
        ...
        #Calculate theoretical Hydrogen flux for each emission line
        He_Flux             = y_plus * Emissivities_module * data_dict['ftau_He_vector'] * CR_Module * f_module
...
#             self.Deblend_He3889A_line(a_H, data_dict['T_4'], cHbeta)
    ...
    def Deblend_He3889A_line(self, a_H, T_4, cHbeta):
        #This algorithm assumes that the abundance 
        print self.flambda_dict[self.He3889_label]
        print self.Flux_dict[self.H13889A_label]
        print self.Flux_dict[self.HBeta_label]
        
        H_He_Obs_Flux = (self.Flux_dict[self.H13889A_label] / self.Flux_dict[self.HBeta_label]) - (0.104 * power(T_4, 0.046)) * (power(10 , - self.flambda_dict[self.He3889_label] * cHbeta))  
        
        return H_He_Obs_Flux     
...
class HeAbundance_InferenceStructure(HeAbundance_InferenceMethods):
    ...
    def Bayesian_HeliumAbundance_Analysis(self):
        ...
        ne          =   pymc.TruncatedNormal(   'n_e',      self.ObjectData_dict['nSII'],     250**-2,    a = 0.0 ,   b = 1000.0)
#         a_He        =   pymc.TruncatedNormal(   'abs_He',   1.0,                                0.5**-2,    a = 0.0,    b = 5.0)
        tau         =   pymc.TruncatedNormal(   'Tau',      0.75,                               0.5**-2,    a = 0.0,    b = 7.0)
        Te          =   pymc.Normal(            'T_e',      self.ObjectData_dict['TOIII'],   2000**-2)
