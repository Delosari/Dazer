...
class HeAbundance_InferenceMethods(Txt_Files_Manager):
    ...
    def Calculate_Synthetic_Fluxes(self, model):
        ...
        H_Flux          = self.H_Flux_theo(self.SyntheticData_dict['xi'], self.SyntheticData_dict['cHbeta'], self.SyntheticData_dict['a_H'], self.Hbeta_EW, Hydrogen_Dict)
        He_Flux         = self.He_Flux_theo_nof(self.SyntheticData_dict['xi'], self.SyntheticData_dict['cHbeta'], self.SyntheticData_dict['a_H'], self.SyntheticData_dict['a_He'], self.SyntheticData_dict['y_plus'], self.Hbeta_EW, Helium_Dict)
    
        print '\nInput Parameters'
        print 'y_plus' ,self.SyntheticData_dict['y_plus']
        print 'n_e', self.SyntheticData_dict['n_e']
        print 'Te_0', self.SyntheticData_dict['Te_0']
        print 'a_H', self.SyntheticData_dict['a_H']
        print 'a_He', self.SyntheticData_dict['a_He']        
    ...
    def H_Flux_theo(self, xi, cHbeta, a_H, Eqw_hbeta, data_dict):
    ...
    def He_Flux_theo_nof(self, xi, cHbeta, a_H, a_He, y_plus, Eqw_hbeta, data_dict):
...
class HeAbundance_InferenceStructure(HeAbundance_InferenceMethods):
    ...
    def Bayesian_HeliumAbundance_Analysis(self):
        ...
        ne          =   pymc.TruncatedNormal(   'n_e',      self.SyntheticData_dict['n_e'],     250**-2,    a = 0.0 ,   b = 1000.0)
        a_He        =   pymc.TruncatedNormal(   'abs_He',   1.0,                                0.5**-2,    a = 0.0,    b = 5.0)
        tau         =   pymc.TruncatedNormal(   'Tau',      0.75,                               0.5**-2,    a = 0.0,    b = 7.0)
        Te          =   pymc.Normal(            'T_e',      self.SyntheticData_dict['TOIII'],   2000**-2)
...
#         xi          =   pymc.DiscreteUniform(           'Xi',       0,      1000)
        ...
        @pymc.deterministic
        def det_HFlux_theo(xi=xi, cHbeta=cHbeta, a_H=a_H, Eqw_hbeta=Eqw_hbeta, data_dict = Calculate_H_LineParameters):
            return self.H_Flux_theo(xi=xi, cHbeta=cHbeta, a_H=a_H, Eqw_hbeta=Eqw_hbeta, data_dict = data_dict)
        
        #Calculate Helium theoretical flux    
        @pymc.deterministic
        def det_He_Flux_theo_nof(xi=xi, cHbeta=cHbeta, a_H=a_H, a_He=a_He, y_plus = y_plus, Eqw_hbeta=Eqw_hbeta, data_dict = Calculate_He_LineParameters):
            return self.He_Flux_theo_nof(xi=xi, cHbeta=cHbeta, a_H=a_H, a_He = a_He, y_plus = y_plus,Eqw_hbeta=Eqw_hbeta, data_dict = data_dict)
